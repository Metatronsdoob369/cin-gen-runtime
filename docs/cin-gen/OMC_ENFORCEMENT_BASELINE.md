...
| Scope-field presence | `spec/GATES.md`; orchestrator ARMED branch | `GovernanceOrchestrator` rejects ARMED contracts when `scope` is missing/falsy | No negative test for missing-scope denial | `PARTIALLY_ENFORCED` | **REUSE** presence check; do not treat as authorization | `server/src/core/orchestrator.ts` ~57–65 |
| Scope-to-action/resource matching | `spec/GATES.md` scope obligations | No matcher binds declared `scope` to the action or resource being executed | No scope-mismatch denial test | `NOT_FOUND` | **BUILD** scope matcher on the mediated path before ARMED Cin-Gen ops | Active-path search; orchestrator never compares scope to target |
| ARMED expiry evaluation on the active spine | `spec/GATES.md` “Expiry Enforced” | Orchestrator requires `expiry` field presence only; no `Date` parse or comparison to now | `tests/governance-gate.spec.ts` does not cover ARMED expiry | `PARTIALLY_ENFORCED` | **REPAIR**: add date-aware expiry on the single mediated spine | Orchestrator ~57–65 |
...
| Core orchestrator audit | GATES “Audit Trail”; `server/src/core/orchestrator.ts` / `audit-logger.ts` | In-memory `auditLog` / event array only; lost on process restart | No durability or denial-receipt tests | `PARTIALLY_ENFORCED` | **REPAIR** before claiming constitutional audit | `orchestrator.ts` ~31,69–74; `server/src/core/audit-logger.ts` |
...
| Audit relationship to the constitutional spine | GATES durable audit claim vs dual loggers | Bridge JSONL and orchestrator memory logs are separate; mutating adapters do not write through the GATES orchestrator audit | No test that state-changing ops emit spine audit | `DISCONNECTED` | **REPAIR**: one mediated path must own denial/execution receipts | Topology: Bridge/LawCRON bypass orchestrator; duplication register D5 |
